...
public abstract class FaaPersistentRoomDatabase extends RoomDatabase implements RoomDatabaseI {
    private static FaaPersistentRoomDatabase INSTANCE;
    ...
    public static FaaPersistentRoomDatabase getDatabase(final Context context){
        // We use the Singleton design pattern so that we only every have one instance of the Room database
        if (INSTANCE == null){
            synchronized (FaaPersistentRoomDatabase.class){
                if (INSTANCE == null){
                    ...
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(),
                            FaaPersistentRoomDatabase.class, "faa_database").allowMainThreadQueries().addCallback(sRoomDatabaseCallback).build();
                    /* Do the following when migrating to a new version of the database
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(),
                            FaaPersistentRoomDatabase.class, "faa_database").addMigrations(MIGRATION_1_2, MIGRATION_2_3).build();
                    */
                }
            }
        }
    ...
    private static class PopulateDbAsync extends AsyncTask<Void, Void, Void> {
        ...
        PopulateDbAsync(FaaPersistentRoomDatabase db){
            catDao = db.getCatDao();
        }
        ...
        protected Void doInBackground(final Void... params) {
            ...
            // Will have to comment this out when running Robolectric Unit tests. Otherwise get a circular dependency
            // caused by a ViewModel
